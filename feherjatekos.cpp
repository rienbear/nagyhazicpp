/**
 * @file feherjatekos.cpp
 * @brief Fehér játékos implementációja a malomjátékhoz
 */
...
/**
 * @brief Fehér játékos konstruktor
 * @details A Jatekos osztály konstruktorát hívja meg
 */
FeherJatekos::FeherJatekos() : Jatekos() {}
 ...
 */
bool FeherJatekos::elhelyezKo(JatekTabla& tabla, int poz) {
    if (kovekSzama > 0 && tabla.pozicioSzabad(poz)) {
        tabla.elhelyezKo(poz, 'W');
        kovekSzama--;
        tabla.tablaKirajzolas();
        std::cout << "Feher jatekos elhelyezett egy kovet a(z) " << poz << ". poziciora." << std::endl;
        return true;
    }
    else {
        tabla.tablaKirajzolas();
        ...
        {
            if (kovekSzama <= 0) {
                std::cout << "Nincs tobb elhelyezheto feher ko!" << std::endl;
            }
            else {
 ...
 */
bool FeherJatekos::mozgatKo(JatekTabla& tabla, int honnan, int hova, int eltavolitva) {
    if (eltavolitva == 6)
    {
        if (tabla.pozicioSzabad(hova) && tabla.azonosSzin(honnan, 'W')) {
            tabla.elhelyezKo(honnan, 'O');
            tabla.elhelyezKo(hova, 'W');
            tabla.tablaKirajzolas();
            std::cout << "Feher jatekos ugaratta a kovet " << honnan << " -> " << hova << std::endl;
            return true;
        }
        else {
            tabla.tablaKirajzolas();
            if ((honnan > 0 && honnan < 25) || (hova > 0 && hova < 25)) {
                ...
                else if (!tabla.azonosSzin(honnan, 'W')) {
                    std::cout << "A(z) " << honnan << ". pozicion nem feher ko van!" << std::endl;
                }
            }
            else
            {
        ...
        if (tabla.pozicioSzabad(hova) && tabla.szomszedosPoz(honnan, hova) && tabla.azonosSzin(honnan, 'W')) {
            tabla.elhelyezKo(honnan, 'O');
            tabla.elhelyezKo(hova, 'W');
            tabla.tablaKirajzolas();
            std::cout << "Feher jatekos mozgatta a kovet " << honnan << " -> " << hova << std::endl;
            return true;
        }
        else {
            tabla.tablaKirajzolas();
 ...
 */
bool FeherJatekos::eltavolitKo(JatekTabla& tabla, int poz) {
    ...
    if (!(tabla.pozicioSzabad(poz)) && tabla.azonosSzin(poz, 'B')) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            if ((malom[0] == poz || malom[1] == poz || malom[2] == poz) &&
                tabla.getKovek()[malom[0]] == 'B' &&
                tabla.getKovek()[malom[1]] == 'B' &&
                tabla.getKovek()[malom[2]] == 'B') {
                malombanVan = true;
                break;
            }
        }
        ...
        if (malombanVan) {
            ...
            bool vanMasik = false;
            for (int i = 1; i <= 24; ++i) {
                if (tabla.getKovek()[i] == 'B') {

                    bool ezIsMalom = false;
                    for (int j = 0; j < 16; ++j) {
                        const int* aktMalom = tabla.getMalom(j);
                        if ((aktMalom[0] == i || aktMalom[1] == i || aktMalom[2] == i) &&
                            tabla.getKovek()[aktMalom[0]] == 'B' &&
                            tabla.getKovek()[aktMalom[1]] == 'B' &&
                            tabla.getKovek()[aktMalom[2]] == 'B') {
                            ezIsMalom = true;
                            break;
                        }
                    }
            ...
            // Ha nincs másik, eltávolíthatjuk
                ...
                eltavolitott++;
                tabla.tablaKirajzolas();
                std::cout << "Feher jatekos eltavolitott egy fekete kovet a(z) " << poz << ". poziciobol." << std::endl;
                return true;
            }
        }
